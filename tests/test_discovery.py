    ...
    def test_inclusion_by_streams(self):
        for stream in self.streams_to_test():
            with self.subTest(stream=stream):
                ...
                # verify that all the non-auromatic fields have inclusion of available
                ...
                with self.subTest(msg="validating non-automatic fields"):
                    self.assertEqual(inclusions_other_than_automatic, {"available"},
                        msg="All non key properties are set to 'available' in metadata",
                        logging="verify all the non-automatic fields are available")
